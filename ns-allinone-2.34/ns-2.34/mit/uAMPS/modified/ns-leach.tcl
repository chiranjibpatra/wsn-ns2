...
Application/LEACH instproc decideClusterHead {} {
  ...
  if {[$self getRandomNumber 0 1] < [expr $thresh] } {
    puts "$nodeID: *******************************************"
...
Application/LEACH instproc advertiseClusterHead {} {
  ...
  puts "Advertise cluster head:$mac_dst $link_dst $ADV_CH msg:$msg $datasize $opt(max_dist) $code_"
}
...
Application/LEACH instproc findBestCluster {} {
  ...
  if [$self isClusterHead?] {
    ...
    puts "ClusterCode:$ClusterCode"
    $ns_ at [expr $now_ + $opt(ra_adv_total)] "$self createSchedule"
  } else {
    ...
    set random_access [$self getRandomNumber 0 [expr $opt(ra_join) - $opt(ra_delay)]]
    $ns_ at [expr $now_ + $opt(ra_adv_total) + $random_access]  "$self informClusterHead"
    $self GoToSleep 
  }
...
Application/LEACH instproc informClusterHead {} {
  ...
  #puts "informClusterhead: $mac_dst $link_dst $JOIN_REQ msg:$msg $datasize $opt(max_dist) $code_"
}
...
Application/LEACH instproc createSchedule {} {
  ...
  if {$numNodes == 0} {
    ...
    set mac_dst [expr 0xffffffff]
    set ADV_SCH [expr 2]
    set link_dst [[$self agent] set source_]
    $self send $mac_dst $link_dst $ADV_SCH $msg $datasize $dist_ $code_
   puts "createSchedule: $mac_dst $link_dst $ADV_SCH msg:$msg $datasize $dist_ $code_"
  }
...
Application/LEACH instproc recvJOIN_REQ {nodeID} {
  ...
  set chID [$self nodeID]
  puts "$chID received notice of node $nodeID at time [$ns_ now]"
  set clusterNodes_ [lappend clusterNodes_ $nodeID]
   puts "clusterhead nodes:$clusterNodes_"
}
...
Application/LEACH instproc SendDataToBS {} {
      ...
      $ns_ at $random_delay "$self send $mac_dst $link_dst $DATA $msg $datasize $dist $opt(bsCode)"
}
