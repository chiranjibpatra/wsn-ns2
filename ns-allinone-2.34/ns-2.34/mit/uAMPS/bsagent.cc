...
void BSAgent::BSsetup(void)
{
  ...
  double *new_ch_X, *new_ch_Y,num_conn,prob;  // possible new CH node coordinates
 ...
 // implementation of Network Evolution  Model  in leach-c
 num_conn=(double)(nn_/p_) - p_ ;
 if((num_conn-1)<=0)
   p_ =p_ ;
 else
{
  prob=0.5((pow((num_conn+3)/(num_conn-1) , 0.5) - 1);
  p_=(int)(nn_*prob);
}
  ...
  double avg_energy=0; 
  for (int i = 0; i < nn_; i++)
    currentE_[i]=2;//setting the energy to be 2joules
  for (int i = 0; i < nn_; i++)
  ...
  FILE *file;
  file=fopen("/home/chiranjib/ns-allinone-2.34/ns-2.34/mit/uAMPS/sims/new_nodes.txt", "r");
  while (i < p_) {
    ...
    if (eligible[randnode]) {
      ...
      if (is_ok) {
        fscanf(file, "%lf", &nodesX_[randnode]);
         fscanf(file, "%lf", &nodesY_[randnode]);
        ch_index[i] = randnode;
  ...
  printf("Min_cost = %f\nlast_iter = %d\nCH are:\n", min_cost, last_iter);
  for (int i=0; i<p_; i++) 
    printf("%f\t%f\t%d\t%f\n", ch_X[i], ch_Y[i], ch_index[i],currentE_[i]);
  fflush(stdout);
